...
class Online_Sequence(Dataset):
    ...
    def __init__(self, seqs, transforms=None,
                 pairs_per_seq=1,sequential_frames=None):
    ...
    def __getitem__(self, index):
        ...
        if len(val_indices)<self.sequential_frames:
            index=np.random.choice(len(self))
            return self.__getitem__(index)

        start=random.randint(0,len(val_indices)-self.sequential_frames)
        val_indices=val_indices[start:start+self.sequential_frames]
