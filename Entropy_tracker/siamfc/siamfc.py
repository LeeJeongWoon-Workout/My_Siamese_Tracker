...
class TrackerSiamFC(Tracker):
    ...
    def __init__(self, net_path=None,actor_path=None,tracker_name=None, **kwargs):
        super(TrackerSiamFC, self).__init__(tracker_name, True)
    ...
    def init(self, img, box):
        ...
        z = ops.crop_and_resize(
            img, self.center, self.target_sz,out_size=127,
        ...
        self.kernel=z
        self.initial_kernel=z

    @torch.no_grad()
    def update(self, img,evaluate=True):

        previous_box = np.array([
        ...
        self.net.eval()
        # search images
        patch_size=np.array([255.,255.])
        x = ops.crop_and_resize(
            img, self.center, patch_size,
            out_size=None,
            border_value=self.avg_color)
            ...
            for it, batch in tqdm(enumerate(dataloader)):
                ...
                for f in range(sequential_frames):
                    ...
                    if f == 0:
                        ...
                        next_state = self.next_state_generator(img)
                        ...
                        if f == sequential_frames - 1:
                            reward=self.reward_calcul(box,ann)
                ...
                if it % 10 == 0:
                    ...
                    torch.save(self.actor.policy.state_dict(), "unsupervised_{}.pth".format(sequential_frames))

    @torch.enable_grad()
    def semi_supervised_learning(self, seqs, val_seqs=None, save_dir='models', sequential_frames=20):
        ...
        for epoch in range(self.cfg.epoch_num):
            ...
            for it, batch in tqdm(enumerate(dataloader)):
                ...
                for f in range(sequential_frames):
                    ...
                    if f == 0:
                        self.init(img, ann)
                        psuedo_bbox.append(ann)

                    else:
                        state, actions, box = self.update(img, evaluate=False)
                        psuedo_bbox.append(box)

                for f in range(sequential_frames-1,-1,-1):
                    img = np.array(images[f].squeeze())

                    if f==sequential_frames-1:
                        self.init(img,psuedo_bbox[f])

                    else:
                        state, action, box = self.update(img, evaluate=False)
                        next_state = self.next_state_generator(img)
                        iou = rect_iou(box, psuedo_bbox[f])
                        ...
                        reward=self.reward_calcul(box,psuedo_bbox[f])
                        transition = [state, action, reward, next_state, 0]

                        epi.append(transition)

                for t in epi:
                        bonus=reward
                        memory.push(t[0],t[1],t[2]+reward,t[3],t[4])
                if len(memory) > 1000:
                    for i in range(self.cfg.number_of_updates):
                        self.actor.update_parameters(memory, batch_size=self.cfg.batch_size)

                if it%10==0:
                    torch.save(self.actor.policy.state_dict(), "semisupervised_{}.pth".format(sequential_frames))
                    print('reward:',reward)
