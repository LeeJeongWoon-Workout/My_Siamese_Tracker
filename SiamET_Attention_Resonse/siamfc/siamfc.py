...
class TrackerSiamFC(Tracker):
    ...
    def __init__(self, net_path=None, **kwargs):
        ...
        self.actor = SAC(num_inputs=2601)
        # setup model
        ...
        #self.actor.load_model('/home/airlab/PycharmProjects/pythonProject5/SiamET4/tools/policy_main.pth')
        self.n=0
    ...
    def get_response(self,img):
        ...
        return response,loc,scale_id

    def bbox_changing(self,loc,scale_id):
    ...
    def update(self, img):
        ...
        response,loc,scale_id=self.get_response(img)
        self.bbox_changing(loc, scale_id)

        # Agent re-tracking
        response,_,_=self.get_response(img)
        sample2 = torch.from_numpy(response)
        s2 = torch.flatten(sample2).unsqueeze(dim=0).cuda()
        state2 = s2.float()
        # state2: torch.Size([1, 2601])

        actions = self.actor.select_action(state2, evaluate=True)
    ...
    def train_update(self, img, evaluate=False):
        ...
        actions = self.actor.select_action(state2, evaluate=False)
        self.center += np.array([actions[0], actions[1]])
        self.target_sz = self.target_sz + self.target_sz * actions[2]
        self.z_sz = self.z_sz + self.z_sz * actions[2]
        self.x_sz = self.x_sz + self.x_sz * actions[2]
                ...
                for i in range(1):
                    for f in range(50):
                        ...
                        if f == 49:
                            ...
                            state, action, box = self.train_update(img, evaluate=False)
