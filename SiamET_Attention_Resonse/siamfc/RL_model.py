...
class D2RLGaussianPolicy(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, num_layers, action_space=None):
        ...
        self.action_scale = torch.FloatTensor([1.,1.,0.05]).to(device)
        self.action_bias = torch.FloatTensor([0.]).to(device)
...
class QNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_dim, num_layers):
        ...
        if num_layers > 2:
            ...
            self.l2_3 = nn.Linear(hidden_dim, hidden_dim)
            self.l2_4 = nn.Linear(hidden_dim, hidden_dim)

        if num_layers > 4:
            self.l1_5 = nn.Linear(hidden_dim, hidden_dim)
            self.l1_6 = nn.Linear(hidden_dim, hidden_dim)

            self.l2_5 = nn.Linear(hidden_dim, hidden_dim)
            self.l2_6 = nn.Linear(hidden_dim, hidden_dim)

        if num_layers == 8:
            self.l1_7 = nn.Linear(in_dim, hidden_dim)
            self.l1_8 = nn.Linear(in_dim, hidden_dim)

            self.l2_7 = nn.Linear(hidden_dim, hidden_dim)
            self.l2_8 = nn.Linear(hidden_dim, hidden_dim)
