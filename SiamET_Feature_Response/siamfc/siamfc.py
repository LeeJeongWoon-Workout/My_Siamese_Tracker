...
class TrackerSiamFC(Tracker):
    ...
    def __init__(self, net_path=None, **kwargs):
        super(TrackerSiamFC, self).__init__('SiamET_response_feature', True)
        self.cfg = self.parse_args(**kwargs)

        # setup GPU device if available
        self.cuda = torch.cuda.is_available()
        self.device = torch.device('cuda:0' if self.cuda else 'cpu')
        self.actor = SAC(num_inputs=3113)
        # setup model
        ...
        self.actor.load_model('/home/airlab/PycharmProjects/pythonProject5/SiamET/tools/policy.pth')
        self.n=0
    ...
    def update(self, img):
        ...
        # Agent
        sample1 = torch.from_numpy(samples[scale_id]).permute(2, 0, 1).unsqueeze(0).float().cuda()
        sample2 = torch.from_numpy(response)

        s1 = self.feature_extractor(sample1)
        s2 = torch.flatten(sample2).unsqueeze(dim=0).cuda()
        state1 = torch.cat([s1, s2], dim=1).float()
        state2 = s2.float()
        # state1: torch.Size([1, 3113])
        # state2: torch.Size([1, 2601])

        actions = self.actor.select_action(state1, evaluate=True)
    ...
    def train_update(self, img, evaluate=False):
        ...
        actions = self.actor.select_action(state1, evaluate=False)
        # locate target center
        disp_in_response = np.array(loc) - (self.upscale_sz - 1) / 2
        disp_in_instance = disp_in_response * \
            self.cfg.total_stride / self.cfg.response_up
